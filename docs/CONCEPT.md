...
- **Modell:** `tags: [String]` pro Session, persistiert; Vorschlagsliste mit Defaults (`to test`, `review`, `blocked`, `wip`), erweitert um alle bereits verwendeten Tags.
- **UI:** Kontextmenü „Tags" auf Session (Sidebar + Tab); Tags erscheinen als kleine Chips in Sidebar, Tab und Quick Switcher.
- **Suche:** Der Quick Switcher matcht auf Tags — `test` findet alles mit Tag „to test".
- **Workflow-Anker:** „to test" ist der Übergabepunkt Mensch↔Agent: Agent fertig → Session taggen → später alle `to test`-Sessions der Reihe nach abarbeiten.

## KI-Assistenz (an-/abschaltbar)

Die App nutzt lokal vorhandene Daten (Claude-Transcripts) plus optional die Claude CLI, um Kontext **zusammenzufassen, zu verstehen und zu ordnen**. Global per Toggle in der UI abschaltbar (persistiert); Default: aus.

**Stufe 1 — deterministisch, immer verfügbar (kein LLM-Call):**
Der Transcript-Reader parst das Session-JSONL (`transcript_path` aus den Hook-Events) und extrahiert den letzten User-Prompt + Claudes letzte Antwort. Claude beendet Turns meist mit einer Zusammenfassung — die letzten Assistant-Zeilen sind daher bereits eine brauchbare Kurzbeschreibung. Angezeigt in Sidebar-Tooltip und Inbox.

**Stufe 2 — LLM-Verdichtung (nur bei Toggle „an"):**
Nach jedem `Stop`/`Notification`-Event (gedrosselt, max. 1×/min pro Session) verdichtet ein `claude -p`-Aufruf (Haiku, headless, nutzt das bestehende Claude-Login — kein API-Key-Management) den Transcript-Tail zu: einer **Ein-Zeilen-Zusammenfassung** („baut Feature-Flag für NIE-4213, wartet auf Permission") und einem **Topic-Label** (ein Wort). Ergebnis als JSON, gespeichert an der Session.

**Stufe 3 — Ordnen & Gruppieren (nur bei Toggle „an", nie automatisch):**
Aus den Topic-Labels schlägt die App eine Gruppierung vor („Nach Themen gruppieren" im KI-Menü): Sessions mit gleichem Topic wandern in eine gemeinsame Gruppe. Vorschläge werden erst nach explizitem Klick angewendet — die KI räumt nie ungefragt um.

**Sicherheits-/Kostenrahmen:** Nur Transcript-Tails (~30 Zeilen) gehen an `claude -p`; alles bleibt auf der Maschine bzw. im bestehenden Claude-Account. Headless-Aufrufe erben `PLANCHETTE_SESSION` nicht → die eigenen Hooks bleiben für sie stumm.
