...
enum L10n {
    ...
    private static let en: [LKey: String] = [
        ...
        .durableSection: "Durable terminals", .durableActive: "New terminals survive a restart", .durableHelp: "Runs each new terminal inside tmux, so its agent keeps working when Planchette quits, crashes or installs an update.", .durableExplanation: "New terminals run inside tmux. Quitting, installing an update or even a crash leaves the agent running — reopening re-attaches to the live session instead of resuming a conversation. A reboot still ends everything. Existing terminals keep what they were created as.", .durableMissingTmux: "Needs tmux, which was not found. Install it (e.g. brew install tmux) and reopen Settings.",
    ...
    private static let nl: [LKey: String] = [
        ...
        .durableSection: "Duurzame terminals", .durableActive: "Nieuwe terminals overleven een herstart", .durableHelp: "Draait elke nieuwe terminal in tmux, zodat de agent doorwerkt wanneer Planchette afsluit, crasht of een update installeert.", .durableExplanation: "Nieuwe terminals draaien in tmux. Afsluiten, een update installeren of zelfs een crash laten de agent doorlopen — bij het heropenen wordt opnieuw verbonden met de levende sessie in plaats van een gesprek te hervatten. Een herstart van de Mac beëindigt alsnog alles. Bestaande terminals blijven zoals ze zijn aangemaakt.", .durableMissingTmux: "Vereist tmux, dat niet gevonden is. Installeer het (bijv. brew install tmux) en open Instellingen opnieuw.",
