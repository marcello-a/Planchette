#!/bin/sh
# Renders a simple app icon (the 🔮 emoji on a dark rounded background) into
# macos/Planchette/Resources/AppIcon.icns. Regenerate only when the icon
# changes. Requires macOS (uses `sips`/`iconutil` and a tiny Swift renderer).
set -eu
